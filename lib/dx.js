...
function DX(gl) {
    ...
    // init
    function setSize(w=gl.canvas.clientWidth, h=gl.canvas.clientHeight) {
    ...
    // program
    function Program(vertexShader, fragmentShader, attrDefs, uniDefs) {
    ...
    function Texture(pixels, width, height) {
        ...
        if (Array.isArray(pixels)) pixels = new Uint8Array(([]).concat(...pixels));
        ...
        if (pixels.constructor === Uint8Array)
    ...
    // buffer
    function Buffer(data, bufferType=null, drawType=null) {

        bufferType = bufferType || "ARRAY_BUFFER";
        drawType = drawType || gl.STATIC_DRAW;

        const TypedArray = {
            "ARRAY_BUFFER": Float32Array,
            "ELEMENT_ARRAY_BUFFER": Uint16Array}
        [bufferType];

        if (data.constructor !== TypedArray) data = new TypedArray(data);

        const buffer = gl.createBuffer();
        gl.bindBuffer(gl[bufferType], buffer);
        gl.bufferData(gl[bufferType], data, drawType);
        return buffer;
    }
    ...
    return {
        gl, clear, draw, setSize,
        Program, Texture, VertexShader, FragmentShader, Buffer
    };
};